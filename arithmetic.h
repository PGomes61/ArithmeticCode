 ...
 * - Entrada: sequência de bytes (@c uint8_t), comprimento @c int (número de octetos).
 * - Saída: buffer de octetos (@c uint8_t) com cabeçalho little-endian e corpo comprimido;
 ...
 * Código desenvolvido no âmbito de trabalho académico. Uso educacional permitido;
...
/** Número de símbolos possíveis por octeto (0–255). */
 ...
 * @param[in] length Número de octetos a considerar.
 * @param[out] raw_counts Vetor de 256 elementos; cada posição i contém quantas vezes o byte i aparece.
 * @note Variáveis globais do ficheiro: nenhuma (função pura sobre os argumentos).
 */
 ...
 * @param[in] length Número de octetos (≥ 0, limitado por @ref MAX_BUFFER).
 ...
 * @note Variáveis globais do ficheiro: nenhuma na API; uso interno de buffers estáticos só na
