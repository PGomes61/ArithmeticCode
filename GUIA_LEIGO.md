# Guia do projeto — explicação para quem está começando

Este documento explica **o que o programa faz**, **palavra por palavra** quando aparecem termos como “símbolo”, e **como o código se organiza**. Não se assume que você conhece C; quando um conceito de programação aparece, há uma nota curta.
...
4. [Lista de arquivos e quem faz o quê](#4-lista-de-arquivos-e-quem-faz-o-quê)
5. [O caminho completo dos dados (do zero ao fim)](#5-o-caminho-completo-dos-dados-do-zero-ao-fim)
6. [O arquivo `main.c` (entrada do programa)](#6-o-arquivo-mainc-entrada-do-programa)
7. [O arquivo `utils.c` (cola e arquivos)](#7-o-arquivo-utilsc-cola-e-arquivos)
8. [O arquivo `arithmetic.c` (o “motor” matemático)](#8-o-arquivo-arithmeticc-o-motor-matemático)
9. [O arquivo `demo.c` (só para mostrar no terminal)](#9-o-arquivo-democ-só-para-mostrar-na-terminal)
...
**Lê uma sequência de bytes (por exemplo texto), calcula estatísticas, transforma num formato comprimido (bits em memória), e permite recuperar exatamente os mesmos bytes.**  
Isso se chama **compressão sem perdas** (lossless): o original e o recuperado são **iguais byte a byte**.
...
`const uint8_t *data` significa: “aponta para bytes que **não será alterado** nesta função”.

### `int length`

**Quantos bytes** serão usados a partir do início do array (o “comprimento da mensagem”).
...
Aqui serve para ter **buffers muito grandes** sem os colocar na **pilha** de uma função (o que poderia “partir” o programa).
...
**Não** é um conceito místico: é só **“qual é o próximo valor de 0 a 255 que estamos tratando neste momento”**.

---

## 4. Lista de arquivos e quem faz o quê

| Ficheiro | Papel em linguagem simples |
|----------|----------------------------|
| `main.c` | Começa o programa. Escolhe: **modo arquivo** ou **demonstração**. |
| `utils.c` | Junta tudo: **modelo → comprimir → verificar com Python**. Lê arquivos. |
...
| `demo.c` | **Imprime** na terminal (pré-visualização) e chama o mesmo pipeline. |
...
Imagine que você tem uma mensagem de bytes na memória.

```
Passo A   Lê os bytes (texto, arquivo ou demo)
    ↓
...
Passo D   Constróis a tabela cumulativa (modelo de probabilidades)
    ↓
Passo E   Comprime: percorre a mensagem byte a byte, estreita um intervalo inteiro,
          escreves bits quando precisas (renormalização WNC)
    ↓
Passo F   Você fica com um "pacote": cabeçalho (n + contagens) + corpo (bits)
    ↓
Passo G   Descomprime: lê o cabeçalho, reconstrói o mesmo modelo,
          lê bits, recupera os n bytes
    ↓
...
## 6. O arquivo `main.c` (entrada do programa)
...
- `./prog entrada.txt` → `argc` é 2, e `argv[1]` é a string `"entrada.txt"` (o caminho do arquivo).

### O que o `main` faz neste projeto?

1. **Se** você passou um nome de arquivo (`argc >= 2`): chama `run_from_file(argv[1])` — lê o arquivo, comprime, verifica com o decoder Python, imprime o original.
...
## 7. O arquivo `utils.c` (cola e arquivos)
...
1. **Valida** ponteiros e tamanhos (se algo for inválido, retorna código de erro negativo).
2. **`arithmetic_build_model_from_data`**: a partir dos bytes, constrói `ArithmeticModel` (cumulativos).
3. **`arithmetic_compress`**: escreve o pacote comprimido num buffer grande **`s_codec_compressed`** (variável `static` no topo do arquivo — existe uma só vez no programa).
...
1. `fopen` abre o arquivo em modo binário (`"rb"`).
2. `fread` copia bytes para **`s_file_input`** (no máximo `MAX_BUFFER`).
3. Se ainda havia bytes no arquivo depois disso, imprime um **aviso** (só leu uma parte).
...
## 8. O arquivo `arithmetic.c` (o “motor” matemático)

Este é o arquivo mais longo. É dividido em **blocos**.
...
Isto permite que o mesmo arquivo binário seja lido da mesma forma em muitas máquinas.
...
- `bw_flush_bit`: coloca o bit num acumulador `cur`; quando chega a 8 bits, escreve um byte em `buf[pos]` e avança `pos`.
...
4. Entra num ciclo **infinito** até não precisar de mais renormalização **neste momento**:
   ...
   - Se nenhuma se aplica, **sai** do laço.
...
A **descompressão** (espelho deste bloco, com leitura de bits) está no arquivo **Python** `scripts/external_arithmetic_decode.py`, não em `arithmetic.c`.
...
- **`arithmetic_scale_counts_for_wnc`**: se a mensagem for **maior** que o limite WNC (`16383`), **reduz** as contagens proporcionalmente para a soma ser `16383`, sem zerar um símbolo que apareceu (mínimo 1). Se a mensagem for **curta**, copia as contagens sem alterar.
...
## 9. O arquivo `demo.c` (só para mostrar no terminal)
...
- O programa C **comprime** e, nas demos / modo arquivo, chama o script **`scripts/external_arithmetic_decode.py`** com `--verify` sobre arquivos temporários.
- Você pode repetir à mão: `./prog --dump-compressed original.txt out.comp` e `python3 scripts/external_arithmetic_decode.py out.comp --verify original.txt`.
...
- **Renormalizar** = **escrever bits** quando já você tem certeza deles e **reabrir margem** no intervalo inteiro, para não ficar sem precisão.
